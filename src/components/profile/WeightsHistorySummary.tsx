...
export function WeightsHistorySummary({
    ...
    const last5Weights =
        weights.length > 3 && currentPath === "/profile"
            ? weights.slice(0, 5)
            : weights;
    ...
    function showHistoryTitle() {
        ...
        return <div className="text-[#4B00FB] text-[13px]">History</div>;
    }
    ...
    return (
        <div className="flex justify-center flex-col mt-2 w-[350]">
            ...
            {last4Weights.map((weight, index) => (
                ...
                >
                    ...
                    >
                        <h1 className="text-[#C8B1FF] text-[14px] pl-2">
                            {new Date(weight.date).toLocaleDateString()}
                        </h1>
                        <h1 className="text-[#4B00FB] text-[14px] pl-2 opacity-90">
                            {convertTimestampToMinutesAndHours(weight.timestamp)}
                        </h1>
                    </div>
                            ...
                            {weights.length > 1 ? (
                                <>
                                    {handleArrow(
                                        calculateDifference(
                                            weight.weight,
                                            last5Weights[index + 1]?.weight || weight.weight
                                        )
                                    )}
                                    <h1 className="">
                    ...
                    {selectedWeightIndex === index && (
                        <AddWeightDialog
                            ...
                            deleteWeightAction={handleDeleteWeightAction}
                            open={true}
